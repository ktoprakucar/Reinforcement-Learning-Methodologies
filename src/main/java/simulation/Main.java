...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        gridWorld.getqTable()[goal.getxAxis()][goal.getyAxis()].setValue(BigDecimal.valueOf(rewardValue));
        if (hasWind)
    ...
    public static void printQTable() {
        for (int j = 0; j < SIZE; j++) {
            for (int i = 0; i < SIZE; i++) {
                System.out.print(gridWorld.getqTable()[i][j].getValue().setScale(9, BigDecimal.ROUND_HALF_UP) + "    ");
            }
            System.out.print("\n");
        }
    }
